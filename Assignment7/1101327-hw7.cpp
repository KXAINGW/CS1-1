...
bool less( HugeInt hugeInt1, HugeInt hugeInt2 )
{
    ...
    for (int i{ hugeInt1.size-1 }; i >= 0; i--)
    {
        if (hugeInt2.ptr[i] > hugeInt1.ptr[i])
            return true;
        else if (hugeInt2.ptr[i] < hugeInt1.ptr[i])
            return false;
    }
    return false;
...
void decrement( HugeInt &hugeInt )
{
    ...
    while (hugeInt.size > 1 && hugeInt.ptr[hugeInt.size - 1] == 0)
        hugeInt.size--;
...
void subtraAssign( HugeInt &minuend, HugeInt subtrahend )
{
   ...
   //difference.size = minuend.size;               //assign處理 
   //difference.ptr = new int[difference.size];    //assign處理 
   ////differt
...
void multiAssign( HugeInt &multiplicand, HugeInt multiplier )
{
    ...
    while (product.size > 1 && product.ptr[product.size - 1] == 0)
        product.size--;
