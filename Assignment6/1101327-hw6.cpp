...
//output hugInt
void check(int* hugeInt, int size)
{
...
int main()
{
    ...
    //int b1[10] = { 7 };
    //int bsize = 1;
    //subtraAssign(a1, b1, asize, bsize);
...
void decrement(int*& hugeInt, int& size)
{
    ...
    assign(buffer, hugeInt, bufferSize, size);
    //assign
    ...
    while (bufferSize > 1 && buffer[bufferSize - 1] == 0)
        bufferSize--;
...
void subtraAssign(int*& minuend, int* subtrahend, int& minuendSize, int subtrahendSize)
{
    ...
    while (differenceSize > 1 && difference[differenceSize - 1] == 0)
        differenceSize--;
    ...
    int dividendSize, int divisorSize, int& quotientSize, int& remainderSize)
{
    ...
    else
    {
        ...
        for (int i = dividendSize - 1; i-n>=0; i--)
