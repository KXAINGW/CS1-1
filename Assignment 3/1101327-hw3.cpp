...
void addition(int addend[], int adder[], int& addendDegree, int adderDegree)
{
    ...
    for (int i{ 0 }; i <= addendDegree; i++)
        addend[i] = addend[i] + adder[i];

    //sutraction -> 可能將degree降低(minus -> addition)
    ...
    int dividendDegree, int divisorDegree, int& quotientDegree, int& remainderDegree)
{
    ...
    {
        int i = (remainderDegree - divisorDegree);
        int buffer[arraySize] = {};
        int bufferDegree = 0;
        //取需要乘的商的係數與次方
        int temp[arraySize] = {};
        ...
        temp[i] = remainder[remainderDegree] / divisor[divisorDegree];

        //cout << "moninomial: ";
        //output(temp, quotientDegree);

        multiplication(divisor, temp, buffer, divisorDegree, quotientDegree, bufferDegree);
        subtraction(remainder, buffer, remainderDegree, bufferDegree);
        //cout << "remainder: ";
        //output(remainder, remainderDegree);
        //cout << remainderDegree << endl;
    }
