...
void createTextFile(fstream& readFromFile)
{
    ...
    //read all
    readFromFile.seekg(0, ios::end);   //ex: 4byte -> end = 4 (0~3byte -> first variable) end = last+1
    int byteSize = readFromFile.tellg();
    int size = byteSize / sizeof(StudentData);
    readFromFile.seekg(0, ios::beg);
    readFromFile.read(reinterpret_cast<char*>(&student), byteSize);
    //adjust size
...
void updateRecord(fstream& updateFile)
{
    ...
    int byteSize = updateFile.tellg();
    int size = byteSize / sizeof(StudentData);
    updateFile.seekg(0, ios::beg);
    updateFile.read(reinterpret_cast<char*>(&student), byteSize);
    int index = -1;
...
void newRecord(fstream& insertInFile)
{
    ...
    int byteSize = insertInFile.tellg();
    int size = byteSize / sizeof(StudentData);
    insertInFile.seekg(0, ios::beg);
    insertInFile.read(reinterpret_cast<char*>(&student), byteSize);
    int index = -1;
    //find 11114XX
    for (int i = 0; i < size; i++)
    {
...
void deleteRecord(fstream& deleteFromFile)
{
    ...
    int byteSize = deleteFromFile.tellg();
    int size = byteSize / sizeof(StudentData);
    deleteFromFile.seekg(0, ios::beg);
    deleteFromFile.read(reinterpret_cast<char*>(&student), byteSize);
    cout << "good(): " << boolalpha << deleteFromFile.good() << endl;
    cout << "fail(): " << boolalpha << deleteFromFile.fail() << endl;
    cout << "eof(): " << boolalpha << deleteFromFile.eof() << endl;
    deleteFromFile.seekg(0, ios::beg);
    cout << "After seekg()\n";
    cout << "good(): " << boolalpha << deleteFromFile.good() << endl;
    cout << "fail(): " << boolalpha << deleteFromFile.fail() << endl;
